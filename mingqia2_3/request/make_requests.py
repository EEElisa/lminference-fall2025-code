...
url = "https://eeelisa--mingqia2-system-3-model-completions.modal.run"
...
async def run_batch_simulation(
    ...
    """
    # Create output directory (caller controls name)
    output_path = Path(output_dir)
    output_path.mkdir(exist_ok=True)
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        tasks = [
            send_batch_at_scheduled_time(
                session, batch, url, start_time, semaphore, str(output_path)
            )
...
async def main():
    parser = argparse.ArgumentParser(description='Run batch inference simulation')
    parser.add_argument(
        '--test-file',
        type=str,
        default='batch_arrivals_sample.json',
        help='Path to batch arrivals JSON (e.g., batch_arrivals_sample.json or batch_arrivals.json)'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default=None,
        help='Output directory for results (default: derived from test file)'
    )
    ...
    # Derive default output dir if not provided
    if args.output_dir:
        output_dir = args.output_dir
    else:
        if args.test_file.endswith("batch_arrivals_sample.json"):
            output_dir = "batch_results_sample_py"
        else:
            output_dir = "batch_results_py"
