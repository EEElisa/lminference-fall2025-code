...
class Block(nn.Module):
    ...
    def forward(self, x, prev_layer_kv=None):
        ...
        attn_output, cur_layer_kv = self.attn(self.ln_1(x), cache=prev_layer_kv)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, past_kv_caches=None): # past_kv_cache is a list of caches, one per layer
        ...
        assert t + t_past <= self.config.block_size, f"Cannot forward sequence of length {t + t_past}, block size is only {self.config.block_size}"
        ...
        for i, block in enumerate(self.transformer.h):
            x, cur_layer_kv = block(x, prev_layer_kv=past_kv_caches[i]) # TODO: adjust this line to use your modified forward() function (and kv)
            # TODO: append to new_kv_caches
            new_kv_caches.append(cur_layer_kv)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens - 1): # TODO: generate just one fewer than max_new_tokens since now we've already generated one (via the prefill block above)
            ...
            logits, _, kv_cache = self(idx_next, past_kv_caches=kv_cache) # TODO: You know the drill by now (if not, do the other functions first)
...
# Run with KV cache (current implementation)
print("=" * 80)
...
with torch.no_grad():
    ...
    output_with_cache = model.generate(sequences, gen_len)
    decoded_with_cache = tokenizer.batch_decode(output_with_cache)
    torch.cuda.synchronize()
    end = time.time()
    time_with_cache = end - start
    print(decoded_with_cache[0])
    print(f"\nGeneration of {gen_len} tokens WITH KV cache took {time_with_cache:.4f} seconds")

# Save output with KV cache
with open('output_with_kv_cache.txt', 'w') as f:
    f.write("=" * 80 + "\n")
    f.write("KV Cache Implementation Results\n")
    f.write("=" * 80 + "\n")
    f.write(f"Generation length: {gen_len} tokens\n")
    f.write(f"Time taken: {time_with_cache:.4f} seconds\n")
    f.write(f"Throughput: {gen_len/time_with_cache:.2f} tokens/second\n")
    f.write("=" * 80 + "\n\n")
    f.write("Generated text:\n")
    f.write(decoded_with_cache[0])
    f.write("\n")

print(f"\n✓ Output saved to 'output_with_kv_cache.txt'")
