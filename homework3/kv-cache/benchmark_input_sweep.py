...
with varying input sequence lengths for Qwen/Qwen3-8B model.
"""
...
def run_sweep(
    ...
    """
    Run the full input length sweep.
    ...
    if input_lengths is None:
        # 2^n for n from 0 to 15
        input_lengths = [2**n for n in range(16)]

    print("="*70)
    print("Starting Input Length Sweep Benchmark")
    print("="*70)
...
def plot_results(results: Dict, output_prefix: str = "benchmark"):
    ...
    input_lens = [m["input_len"] for m in successful]
    ...
    ax1.plot(input_lens, times, 'o-', linewidth=2, markersize=8, color='#2E86AB')
    ax1.set_xlabel('Input Sequence Length (tokens)', fontsize=12)
    ax1.set_ylabel('Wall-clock Time (seconds)', fontsize=12)
    ax1.set_title('Wall-clock Time vs Input Length', fontsize=14, fontweight='bold')
    ax1.set_xscale('log', base=2)
    ax1.grid(True, alpha=0.3)
    ax1.set_xticks(input_lens)
    ax1.set_xticklabels([str(x) for x in input_lens], rotation=45)

    # Mark OOM points
    if oom_points:
        oom_lens = [m["input_len"] for m in oom_points]
        ax1.axvline(x=min(oom_lens), color='red', linestyle='--', alpha=0.5, label='OOM')
        ax1.legend()

    # Plot 2: Throughput
    ax2.plot(input_lens, throughputs, 's-', linewidth=2, markersize=8, color='#A23B72')
    ax2.set_xlabel('Input Sequence Length (tokens)', fontsize=12)
    ax2.set_ylabel('Throughput (tokens/second)', fontsize=12)
    ax2.set_title('Token Throughput vs Input Length', fontsize=14, fontweight='bold')
    ax2.set_xscale('log', base=2)
    ax2.grid(True, alpha=0.3)
    ax2.set_xticks(input_lens)
    ax2.set_xticklabels([str(x) for x in input_lens], rotation=45)
...
def print_table(results: Dict):
    ...
    print(f"Batch size: {results['batch_size']}, Output length: {results['output_len']} tokens")
    print("="*70)
    print(f"{'Input Length':<15} | {'Time (s)':<12} | {'Throughput (tokens/s)':<25} | {'Status':<10}")
    print("-"*70)
    ...
    for m in results["measurements"]:
        ...
        print(f"{input_len:<15} | {time_str:<12} | {throughput_str:<25} | {status:<10}")
...
if __name__ == "__main__":
    ...
    # Input lengths to test: 2^n for n from 0 to 15
    INPUT_LENGTHS = [2**n for n in range(16)]
    ...
    save_results(results, "input_sweep_results.json")
    print_table(results)
    plot_results(results, "input_sweep")
